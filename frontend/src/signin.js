...
class Home extends Component {
  ...
  onSignIn() {
    ...
    // Post request to backend
   axios.post('http://localhost:3201/api/account/signin/'+ this.state.signInEmail +"/" +this.state.signInPassword)
      .then(res => {
        console.log('json', res);
        if (res.success) {
  ...
  logout() {
    ...
    if (obj && obj.token) {
      const { token } = obj;
      // Verify token
      axios('http://localhost:3201/api/account/logout?token=' + token)
